...
bool CoxSurface::init(const JParser& prm) {
  ...
  do_hsd = (prm.getStringOrEmpty("type") == "hsd");
  if (do_hsd) {
    height_sd = prm.getDouble("height_sd");
  } else {
...
void CoxSurface::buildSurface() {
  ...
  if (!do_hsd) {
    ...
    epsilon = 0.866; // equilateral triangles
    sigma = height_sd;
  }
...
void CoxSurface::checkSlopes() {
  ...
  jlog::ls.printf("along_slope_sd %f (should be %f)\n", along_sd, (do_hsd ? sqrt(2) * height_sd : sqrt(au*wind_speed)));
  jlog::ls.printf("mean_across_slope %f\n", mean_across_slope);
  jlog::ls.printf("across_slope_sd %f (should be %f)\n", across_sd, (do_hsd ? sqrt(2) * height_sd : sqrt(ac*wind_speed)));
