...
InterfaceWidget::InterfaceWidget(QWidget* parent) : 
  ...
  iface_sd_rb = new QRadioButton("Height S.D.:", this);
  button_grp.addButton(iface_sd_rb);
  grid->addWidget(iface_sd_rb, 0, 0);
  height_sd_le = new QLineEdit(this);
  grid->addWidget(height_sd_le, 0, 1);
  height_sd_le->setFixedWidth(70);

  iface_wb_rb = new QRadioButton("Wind speed:", this);
  button_grp.addButton(iface_wb_rb);
  grid->addWidget(iface_wb_rb, 1, 0);
  wind_speed_le = new QLineEdit(this);
  grid->addWidget(wind_speed_le, 1, 1);
  wind_speed_le->setFixedWidth(70);
  wb_label3 = new QLabel("(ms-1)", this);
  grid->addWidget(wb_label3, 1, 2);
 
  wb_label1 = new QLabel("Direction:", this);
  grid->addWidget(wb_label1, 2, 0, 1, 1, Qt::AlignRight);
  wind_direc_le = new QLineEdit(this);
  grid->addWidget(wind_direc_le, 2, 1);
  wind_direc_le->setFixedWidth(70);
  wb_label2 = new QLabel("(degrees),   or:", this);
  grid->addWidget(wb_label2, 2, 2);
  azavg_cb = new QCheckBox("Azimuthally average");
  grid->addWidget(azavg_cb, 2, 3);
...
void InterfaceWidget::parserToGui(const JString& prefix, const JParser& prm, prm_context pc) {
  ...
  if (prm.isDefined(prefix+"type")) {
    ...
    else if (prm.valueCheck(prefix+"type", "hsd")) iface_sd_rb->setChecked(true);
  ...
  s = prm.getStringOrEmpty(prefix+"height_sd");
  height_sd_le->setText(J2Q(s));
...
void InterfaceWidget::guiToParser(const JString& prefix, JParser& prm, prm_context pc) const {
  ...
  else if (iface_sd_rb->isChecked()) prm.setLabelAndValue(prefix+"type", "hsd");
  ...
  if (!(s = iface_flat_refrac1_le->text()).isEmpty()) prm.setLabelAndValue(prefix+"refrac_index_1", s);
    ...
    // height standard deviation
  if (!(s = height_sd_le->text()).isEmpty()) prm.setLabelAndValue(prefix+"height_sd", s);
    // wind speed and direction
...
void InterfaceWidget::slotUpdateEnabled() {
  ...
  refrac_widget->setEnabled(iface_flat_rb->isChecked() || iface_sd_rb->isChecked() || iface_wb_rb->isChecked());

  bool f1 = iface_sd_rb->isChecked();
  height_sd_le->setEnabled(f1);
  
  bool f2 = iface_wb_rb->isChecked();
  wb_label1->setEnabled(f2);
  wb_label2->setEnabled(f2);
  wb_label3->setEnabled(f2);
  wind_direc_le->setEnabled(f2 && !azavg_cb->isChecked());
  wind_speed_le->setEnabled(f2);

  azavg_cb->setEnabled(f2);
}
